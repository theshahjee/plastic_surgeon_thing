...
def mark_coordinates_on_image(image_path, coordinates_list):
    ...
    # Loop through the coordinates and mark each with a larger red dot and a number
    for index, point in enumerate(coordinates_list):
        coordinates = point["coordinates"]
        x, y = coordinates[0], coordinates[1]

        # Draw a larger red circle (dot) at each coordinate on the image
        radius = 10  # Increased radius for larger dot
        cv2.circle(image, (x, y), radius, color=(0, 0, 255), thickness=-1)  # Red dot

        # Put the index number inside the red dot
        font_scale = 0.6  # Font scale for text size
        thickness = 2  # Thickness of the text
        text_position = (x - 5, y + 5)  # Adjust position slightly for centering
